...
export default function NewProductPage() {
  ...
  const [brand, setBrand] = useState('OSMIUM');
  const [subcategory, setSubcategory] = useState('');
  const [imageUrl, setImageUrl] = useState('');
  const [description, setDescription] = useState('');
  const [attributes, setAttributes] = useState([{ key: 'color', value: 'Matte Obsidian' }]);
  ...
  const handleSubmit = async (e) => {
    ...
    const productPayload = {
      ...
      brand,
      subcategory,
      images: [imageUrl || 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?auto=format&fit=crop&w=800&q=80'],
      description,
      attributes: attrMap,
      status: 'active',
    };

    const res = await createProductAdmin(productPayload);
    setIsLoading(false);

    if (res.success) {
      setToastMessage('Product successfully added to catalog');
      setTimeout(() => {
        ...
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Input
                label="Product Name"
                placeholder="e.g. OSMIUM Precision Speaker"
                value={name}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Input
                ...
                placeholder="e.g. Audio / Footwear / Lighting"
                value={subcategory}
            ...
            {/* Description */}
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-mono tracking-wider text-secondary uppercase">
                Product Description
              </label>
              <textarea
                className="w-full bg-obsidian text-primary border border-border rounded-md px-3.5 py-2 text-sm focus:border-cyan focus:ring-1 focus:ring-cyan focus:outline-none min-h-[100px]"
            ...
            <div className="flex flex-col gap-3 pt-4 border-t border-border">
              ...
              {attributes.map((attr, idx) => (
                <div key={idx} className="flex items-center gap-3">
                  ...
                  />
                  <Input
                    placeholder="Value (e.g. Full-Grain Calfskin)"
                    value={attr.value}
            ...
            {/* Submit */}
            <Button type="submit" variant="primary" size="lg" isLoading={isLoading} icon={Save} className="mt-4">
